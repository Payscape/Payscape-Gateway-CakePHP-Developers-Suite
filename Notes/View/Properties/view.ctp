...
//    $main_image = Router::url('/', true).'img/'.my_ife( $image['image']['main'], 'manage_missing.png' );
	...
	//  $image_path = 'vacationfish.localdomain/public_html/webroot/img/files/' . $image['property_id'] . '/images/' . $image['image'];
	    $image_path = WWW_ROOT . '/img/files/' . $image['property_id'] . '/images/' . $image['image'];
...
// display the file if it exists
    ...
    if(is_file($imagepath)==TRUE){
    	echo "<br>NAME not empty, FILE EXISTS: $image_path<br>";
    ...
    /*
    	...
    	echo "<br>DOES NOT EXIST: $image_path";
    } 
*/
                                    } // not empty image['name']
                                    else {
// echo $this->Html->link(  $this->Html->image( my_ife($image['image']['thumb'], 'manage_missing.png'), array('alt' => '')), $main_image, array('escape' => false) );
	...
	if(file_exists($image_path)==TRUE){
		echo "<br>NAME is empty, FILE EXISTS: $image_path<br>";
	
                    // echo $this->Html->link(	 $this->Html->image( 'files/'. $image['property_id'] . '/images/' . my_ife( $image['image'], 'manage_missing.png'), array('data-title' => $property['Property']['title'], 'data-description' => 'Vacation Fish premium', 'alt' => $property['Property']['title'])), $main_image, array('escape' => false) );
			}
/*
			 else {
		echo "<br>NOT A FILE: $image_path";
	}
*/                                  	
 // echo $this->Html->link( $this->Html->image( 'files/'. $image['property_id'] . '/images/' . my_ife( $image['image'], 'manage_missing.png'), array('alt' => 'Vacation Fish premium listing')), $main_image, array('escape' => false) );
                            ...
                            } // ! empty PropertyImage[0]['image']
                            else {
        echo $this->Html->image('./img/main_missing.png');
                            }
                        ?>
                        </div>    
					</div>
