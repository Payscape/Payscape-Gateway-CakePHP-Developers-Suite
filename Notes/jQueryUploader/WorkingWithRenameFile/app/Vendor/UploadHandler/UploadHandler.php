...
class UploadHandler
{
    ...
    function __construct($options, $initialize = true, $error_messages = null) {
        $this->options = array(
...
/* bof rename */       
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            $file_path = $upload_dir.$changed_name;
/* eof file rename */            
    ...
    public function delete($print_response = true) {
        $file_name = $this->get_file_name_param();
        $file_path = $this->get_upload_path($file_name);
    //    $file_path = 'img/files/'.$property_id.'/images/'.$filename;
    // , $property_id
        $success = is_file($file_path) && $file_name[0] !== '.' && unlink($file_path);
        $this->deleteImage($property_id, $file_name);
        if ($success) {
            foreach($this->options['image_versions'] as $version => $options) {
                if (!empty($version)) {
    ...
    public function saveImage($property_id, $file){
        if((isset($property_id))&&(isset($file))){
            ...
            $this->$model->PropertyImage->set('sort', 4);
            $this->$model->PropertyImage->set('image', $file);
            $this->$model->PropertyImage->set('property_id', $property_id);
            $this->$model->PropertyImage->save();
        }
